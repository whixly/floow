...
-- Points reset daily (Philippine time, UTC+8) — only counts TODAY's activity
...
AS $$
  ...
  LEFT JOIN (
    ...
    WHERE session_type = 'work'
      AND (completed_at AT TIME ZONE 'Asia/Manila')::date = (NOW() AT TIME ZONE 'Asia/Manila')::date
    GROUP BY user_id
  ...
  LEFT JOIN (
    ...
    FROM public.tasks
    WHERE status = 'done'
      AND (updated_at AT TIME ZONE 'Asia/Manila')::date = (NOW() AT TIME ZONE 'Asia/Manila')::date
    GROUP BY user_id
